...
import java.time.LocalDate; // Importa la clase Estudiante del paquete model
import java.util.ArrayList; // Importa la anotación Repository de Spring
import java.util.List; // Importa la clase LocalDate para manejar fechas
import java.util.Map; // Importa la clase ArrayList para manejar listas
import java.util.concurrent.ConcurrentHashMap; // Importa la interfaz List para manejar listas
import java.util.concurrent.atomic.AtomicLong; // Importa la interfaz Map para manejar mapas

import org.springframework.stereotype.Repository; // Importa la clase ConcurrentHashMap para manejar mapas concurrentes

import com.universidad.model.Estudiante; // Importa la clase AtomicLong para manejar contadores atómicos
...
public class EstudianteRepository {
    ...
    public Estudiante findById(Long id) {
        return estudiantes.get(id); // Retorna el estudiante correspondiente al ID o null si no lo encuentra
    }
    ...
    public void init() {
        Estudiante estudiante1 = Estudiante.builder() // Crea un estudiante usando el patrón builder
                .nombre("Jose") // Asigna el nombre
                .apellido("Pérez") // Asigna el apellido
                .email("j.perez@hotmail.com") // Asigna el email
                .fechaNacimiento(LocalDate.of(2001, 5, 17)) // Asigna la fecha de nacimiento
                .numeroInscripcion("0001") // Asigna el número de inscripción
        ...
        Estudiante estudiante2 = Estudiante.builder() // Crea otro estudiante usando el patrón builder
                ...
                .apellido("Magdalena") // Asigna el apellido
                .email("maria.mm@gmail.com") // Asigna el email
                .fechaNacimiento(LocalDate.of(2007, 8, 22)) // Asigna la fecha de nacimiento
                .numeroInscripcion("0002") // Asigna el número de inscripción
                .build(); // Construye el objeto Estudiante
